...
export default function CatalogPage() {
  ...
  return (
    <Container className={cn("mt-4 flex flex-col gap-2")}>
      <div
        className={cn(
          "flex flex-col gap-8 fixed top-55 left-4  z-50 max-sm:hidden"
        )}
      >
        {linkArr.map((l, index) => (
      ...
      <Products products={phones} className="max-w-500  pl-8  max-sm:pl-0" />
      ...
      <Products products={nouts} className="max-w-500 pl-8 " />
    </Container>
  );
}
